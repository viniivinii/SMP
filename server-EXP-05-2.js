...
// Criar nova embalagem (blister ou caixa)
...
app.delete("/embalagens/:id", async (req, res) => {
  ...
  try {
    await db.execute("DELETE FROM estoque.embalagem WHERE id = ?", [embalagemId]);
    res.json({ success: true });
  } catch (err) {
...
app.put("/itens/:id", async (req, res) => {
  ...
  try {
    await db.execute("UPDATE estoque.itens_separados SET status = ? WHERE id = ?", [status, id]);
    res.json({ success: true });
  } catch (err) {
...
// Rota para atualizar embalagens (usando UPDATE)
